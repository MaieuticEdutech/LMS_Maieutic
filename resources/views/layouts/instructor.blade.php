{{--
    ...
    Brand pass (docs/UI-GUIDE.md §7 groups admin and instructor under the
    same "dark sidebar" treatment): identical structure to layouts/admin.blade.php
    — 248px sticky teal-900 sidebar, off-canvas drawer below 1024px. Nav
    content lives in layouts/partials/instructor-nav.blade.php so the
    desktop aside and the mobile drawer render identically from one source.
--}}
<!DOCTYPE html>
...
<head>
    ...
    <title>@yield('title', 'Instructor') &middot; {{ app(\App\Services\Settings\BrandingService::class)->organisationName() }}</title>
...
<body class="min-h-full bg-neutral-50 text-neutral-800 antialiased" x-data="{ drawerOpen: false }">
    ...
    <div class="flex min-h-full">
        ...
        <aside class="sticky top-0 hidden h-screen w-sidebar shrink-0 flex-col bg-teal-900 lg:flex">
            @include('layouts.partials.instructor-nav')
        </aside>
            ...
            <header class="border-b border-neutral-200 bg-neutral-0">
                <div class="flex items-center gap-3 px-4 py-3 sm:px-6">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <h1 class="truncate text-base font-semibold text-neutral-900">@yield('heading', 'Instructor')</h1>
                    </div>

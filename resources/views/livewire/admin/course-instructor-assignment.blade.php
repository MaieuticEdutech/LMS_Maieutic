<div>
    <x-card title="Assigned courses">
        @if ($assignedCourses->isEmpty())
            <p class="text-sm text-neutral-500">Not assigned to any courses yet.</p>
        @else
            <ul class="divide-y divide-neutral-100">
                @foreach ($assignedCourses as $course)
                    <li wire:key="assigned-course-{{ $course->id }}" class="flex items-center justify-between py-2">
                        <span class="text-sm font-medium text-neutral-900">{{ $course->title }}</span>
        ...
        @if ($availableCourses->isNotEmpty())
            <div class="mt-4 flex items-end gap-2 border-t border-neutral-200 pt-4">
                <div class="flex-1">

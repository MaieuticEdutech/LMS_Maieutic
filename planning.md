...
| **Phase 11 status** | 🟡 **Everything buildable is built, 2026-08-13.** Infrastructure in PR #6; the rest completed once Phases 8 and 9 shipped their triggers — `AssessmentResult` and `CourseCompleted` emails, the admin **email log viewer** and the admin **queue-health panel** with failed-job retry. **8 of 10 mailables.** `PurchaseConfirmation` and `PaymentFailed` need a verified payment, which is Phase 12; building them now would be two mailables no code path can reach (Rule 5). Correctly NOT marked done. |
...
| Quality gates | Pint clean · Larastan **level 8**, 0 errors · Pest **1185/1185**, 2,530 assertions, **zero warnings** — measured on `phase/08-11-close-remaining-gaps` off `main` (`914502c`), so the figure includes every track's work together |
...
| Outstanding verification items | (1) ~~CI never executed~~ — 🟢 **CLOSED**, green on `7993da5`. (2) **Vite HMR not interactively verified** — still open, does not block anything. (3) **Branch protection not enabled on `main`** — until it is, the PR rules in §21.6 are documentation rather than enforcement. (4) **Three DoD items need a human, not a test** — the Phase 4 dashboard under 400 ms, Phase 6 video seeking in a real browser, Phase 7 at 360 px. Marked `[~]` in `phases.md` so they read as known-outstanding rather than forgotten. (5) **`origin/phase/04-brand-pass` is still on the remote** — the abandoned duplicate brand work from PR #18; its owner should delete it. |
...
| 8 | Assessment Engine | 🟢 Complete | 2026-08-13 | Track B, PR #23. Authoring, attempt runner, grading, `AttemptGraded`. **Behavioural test suite added on the completeness pass** — it found three live bugs: quiz authoring threw on PostgreSQL, autosave could not create a first answer row outside production, and `time_spent_seconds` was always zero |
...
| 10 | Instructor Module | 🟢 Complete | 2026-08-13 | PR #25, in two commits: the progress-free slice (dashboard, assigned courses, course detail, assessment results) then the progress half once Phase 9 landed on `main` (`StudentProgressDetail`). `Assessments\Results` is shared with the admin area — one implementation, chrome chosen dynamically. AC-03 and the instructor financial-isolation rule covered on the completeness pass |
| 11 | Queues, Mail & Notifications | 🟢 Complete | 2026-09-21 | PR #6 built the infrastructure; Phases 8/9 added `AssessmentResult`/`CourseCompleted`, the email log viewer and queue-health panel. The last two, `PurchaseConfirmation` and `PaymentFailed`, landed with Phase 12's verified payment — all 10 of 10 emails now exist |
| 12 | Payments & Automated Enrollment | 🟡 **Core purchase flow complete** | 2026-09-21 | Checkout (guest + logged-in), server-side pricing, `PaymentGateway`/`RazorpayGateway`/`FakeGateway`, signature-verified idempotent webhook, `SettleCapturedPayment`/`RecordFailedPayment` as the only purchase-side callers of `GrantEnrollment`, `lms:orders:reconcile` + `lms:orders:cancel-abandoned` on the scheduler, purchase-confirmation/payment-failed emails. 33 tests, full suite 1705/1705. **Outstanding against phases.md's DoD:** admin order/payment/webhook-event screens, student payment-history screen, and refund-webhook handling (`AC-15`) — deferred, refunds are manual in the Razorpay dashboard per A-14, not yet built as an automated webhook path. **Blocked on PD-09 test credentials** for the one thing that can't be proven with `FakeGateway`: an actual round-trip against Razorpay. Not marked 🟢 — DoD is not yet satisfied (Rule 25) |

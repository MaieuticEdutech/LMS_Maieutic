...
return [
    ...
    */
    ...
    /*
     * ═════════════════════════════════════════════════════════════════════
     * THIS FILE EXISTS FOR ONE REASON: LIVEWIRE'S 12MB DEFAULT.
     *
     * Livewire validates every temporary upload with `max:12288` (12 MB)
     * before the application sees the file. That silently overrode
     * `lms.media.max_bytes.video`, which allows 2 GB — a 334 MB lecture was
     * rejected at the framework boundary with the app's own ceiling never
     * consulted.
     *
     * The value is duplicated from `lms.media.max_bytes.video` rather than
     * read from it — see the note on the rule below for why a config() call
     * cannot work here. `MediaUploadLimitTest` fails if the two drift apart.
     *
     * The app ceiling remains the real gate: `FileValidationService` still
     * enforces per-purpose size, extension and magic-byte checks after the
     * upload lands (AC-21). This only stops the framework refusing the file
    ...
    'temporary_file_upload' => [
        ...
        /*
         * Kilobytes, and sized to the LARGEST purpose the system accepts —
         * this rule cannot know which purpose an upload is for, so anything
         * smaller would reject a legitimate video. Per-purpose limits are
         * applied afterwards by FileValidationService.
         *
         * Read from the environment rather than through config('lms...'):
         * config files are evaluated while the config repository is still
         * being built, so a config() call here resolves to the default and
         * silently reinstates the 12 MB cap. Keep this value in step with
         * `lms.media.max_bytes.video`; MediaUploadLimitTest asserts they agree.
         */
        'rules' => ['required', 'file', 'max:'.(int) env('LIVEWIRE_MAX_UPLOAD_KB', 2 * 1024 * 1024)],
        ...
        /*
         * 5 minutes is not enough for a multi-hundred-megabyte lecture on a
         * typical Indian upstream connection: 334 MB at 2 Mbps is over 20
         * minutes, and the upload would be invalidated mid-flight with no
         * useful error. 60 minutes covers the 2 GB ceiling at a realistic rate.
         */
